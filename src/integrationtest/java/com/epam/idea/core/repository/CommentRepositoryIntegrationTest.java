...
public class CommentRepositoryIntegrationTest {
    ...
    public void shouldFindAllCommentsByUserId() throws Exception {
        // When:
        final List<Comment> comments = this.commentRepository.findByUserId(USER_ID);

        // Then:
        assertThat(comments).hasSize(1);
        assertThatComment(comments.get(0)).hasId(COMMENT_ID).hasBody(COMMENT_BODY).hasRating(COMMENT_RATING);
        assertThatUser(comments.get(0).getAuthor()).hasId(USER_ID).hasUsername(USERNAME).hasEmail(USER_EMAIL);
    }
}
