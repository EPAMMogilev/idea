...
public class IdeaRepositoryIntegrationTest {
    ...
    public static final State IDEA_STATE = State.New;
    ...
    public void shouldFindAllIdeasByUserId() throws Exception {
        ...
        assertThatIdea(ideas.get(0)).hasId(IDEA_ID).hasTitle(IDEA_TITLE).hasDescription(IDEA_DESCRIPTION)
                .hasState(IDEA_STATE).hasRating(IDEA_RATING);
        assertThatUser(ideas.get(0).getAuthor()).hasId(USER_ID).hasUsername(USERNAME).hasEmail(USER_EMAIL);
    }
    ...
    public void shouldFindAllIdeasByTagId() throws Exception {
        ...
        assertThatIdea(ideas.get(0)).hasId(IDEA_ID).hasTitle(IDEA_TITLE).hasDescription(IDEA_DESCRIPTION)
                .hasState(IDEA_STATE).hasRating(IDEA_RATING);
        assertThatTag(ideas.get(0).getRelatedTags().get(0)).hasId(TAG_ID).hasName(TAG_NAME);
    }
